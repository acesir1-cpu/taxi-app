...
export default function ActiveRideMap({
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] ${className ?? ''}`}
    >
      <MapContainer
        center={[driverPos.lat, driverPos.lng]}
        zoom={13}
        zoomControl={false}
        className="active-ride-map-canvas h-72 w-full sm:h-96"
        scrollWheelZoom
      >
